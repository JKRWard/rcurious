...
output :
  #ioslides_presentation
   ...
   html_document: 
   # Toggle this and comment out ioslides_presentation.
     toc: true
     toc_float: true
...
```{r reading data}
rawd <- read_csv("https://raw.githubusercontent.com/JakeRuss/witch-trials/master/data/trials.csv")
...
We havent told ggplot what is x and what is y 
## Aesthetics
...
my_map <- rawd %>% 
  leaflet() %>% 
  addTiles() %>% 
  addCircleMarkers(lng = ~lon, lat = ~lat, color= ~pal(country), radius = ~deaths/10, opacity = .5)
```
